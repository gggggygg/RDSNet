...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB, rOBB)) else self.forward(x)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, rC2f, RDSC2f, r_customC2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3):
        ...
        elif m in (Detect, WorldDetect, Segment, Pose, OBB, rOBB, ImagePoolingAttn):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "pose"
            elif isinstance(m, OBB) or isinstance(m, rOBB):
                return "obb"
