...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=4):
        ...
        self.n = n
        self.coord_cache = {}  # {(imgsz, index): (mask, src_x, src_y)}
        self.rdial_distortion_cache = OrderedDict()
    ...
    def _mix_transform(self, labels):
        ...
        return (
            self._mosaic4_fish(labels) if config.mosaic_fish  else self._mosaic3(labels) if self.n == 3 else self._mosaic4(labels) if self.n == 4 else self._mosaic9(labels)
    ...
    def warp_image_to_fisheye_sector(self, img4, patch_box, theta_0=np.pi / 2, center=None, index = 0):

        def compute_inverse_mapping_full(imgsz, patch_box, theta_0, center, index = 0):
            """
            对整个图像大小 imgsz × imgsz 遍历每个像素位置，计算其反向鱼眼映射是否落在 patch_box 中。
            patch_box: (x1a, y1a, x2a, y2a)
            return: mask, src_x, src_y
            """
            H, W = imgsz, imgsz
            ...
            if index==0:
                dx = xs - center[1]
                dy = center[0] - ys  # y轴朝下，需倒转

                r = np.sqrt(dx ** 2 + dy ** 2)
                theta = np.arctan2(dx, dy)  # dx在前，角度 ∈ [-pi, pi]

                # 判断哪些点在这个扇形范围内（鱼眼张角）
                valid_angle = (theta >= -theta_0 / 2) & (theta <= theta_0 / 2)

                # 反向映射到 patch 坐标
                dis_y = r
                dis_x = ((theta + theta_0 / 2) / theta_0) * s #目前进行整体畸变

                src_x = dis_x  #相对点（2s,s)的偏移
                src_y = dis_y

                src_x = s + src_x  # 平移到 patch 原图全图坐标
                src_y = s*2 - src_y  # 平移到 patch 原图全图坐标（y轴朝下）

                # 判断哪些点映射后会落在 patch 的有效区域内
                valid_coord = (src_x >= s) & (src_x < 2*s - 1) & (src_y >= 0) & (src_y < s - 1)

                # mask：整个图像中哪些点是来自该 patch 的扇形区域
                mask = valid_angle & valid_coord
                ...
                r = np.sqrt(dx ** 2 + dy ** 2)
                theta = np.arctan2(dy, dx)  # dx在前，角度 ∈ [-pi, pi]

                # 判断哪些点在这个扇形范围内（鱼眼张角）
                valid_angle = (theta >= -theta_0 / 2) & (theta <= theta_0 / 2)

                dis_x = r
                dis_y = ((theta + theta_0 / 2) / theta_0) * s #目前进行整体畸变

                src_x = s + dis_x
                src_y = s + dis_y

                # 判断哪些点映射后会落在 patch 的有效区域内
                valid_coord = (src_x >= 2*s) & (src_x < 3*s - 1) & (src_y >= s) & (src_y < 2*s - 1)
                ...
                # 判断哪些点映射后会落在 patch 的有效区域内
                valid_coord = (src_x >= 0) & (src_x < s - 1) & (src_y >= s) & (src_y < 2*s - 1)
                ...
                # 判断哪些点映射后会落在 patch 的有效区域内
                valid_coord = (src_x >= s) & (src_x < 2*s - 1) & (src_y >= 2*s) & (src_y < 3*s - 1)

                # mask：整个图像中哪些点是来自该 patch 的扇形区域
                mask = valid_angle & valid_coord


            return mask, src_x.astype(np.float32), src_y.astype(np.float32)

        s = self.imgsz

        # 输入为图像大小 imgsz 和该 patch 的位置 bbox、圆心和张角
        mask, src_x, src_y = compute_inverse_mapping_full(
            imgsz=s*3,
            patch_box=patch_box,
            theta_0=theta_0,
            center=center,
        ...
        # # 使用 OpenCV remap 或自定义 bilinear_sample
        # warped = img4.copy()
        # for c in range(3):
        #     warped[..., c][mask] = cv2.remap(img4[..., c], src_x, src_y, interpolation=cv2.INTER_LINEAR, borderValue=114)[mask]
        #
        # return warped

    def warp_segments_to_fisheye(self, segments, patch_box, theta_0, center, imgsz, padw, padh, nw, nh, index = 0):
        """
        """

        s = self.imgsz  # mosaic patch size

        # 拷贝新 segments
        segs = segments.copy()
        x = segs[..., 0]*nw + padw  # 将 segments 坐标平移到 patch 原图全图坐标
        y = segs[..., 1]*nh + padh  # 将 segments 坐标平移到 patch 原图全图坐标

        # segments: [N, M, 2]

        if index == 0 or index == 3: #上与下

            segments_h = y.max(axis=1) - y.min(axis=1)  # [N]
            x_min = x.min(axis=1)  # [N]
            x_max = x.max(axis=1)  # [N]
            w = x_max - x_min  # [N]

            ratio = 0.5 * segments_h / s  # [N]
            x_left = x_min + w * ratio / 2  # [N]
            x_right = x_max - w * ratio / 2  # [N]

            # 扩展维度用于广播
            x = x  # shape: [N, M]
            x_left = x_left[:, None]  # shape: [N, 1]
            x_right = x_right[:, None]  # shape: [N, 1]

            x = np.clip(x, x_left, x_right)  # 防止框过宽

        elif index == 1 or index == 2: #右与左

            segments_w = x.max(axis=1) - x.min(axis=1)  # [N]
            y_min = y.min(axis=1)  # [N]
            y_max = y.max(axis=1)  # [N]
            h = y_max - y_min  # [N]

            ratio = 0.5 * segments_w / s  # [N]
            y_top = y_min + h * ratio / 2  # [N]
            y_bottom = y_max - h * ratio / 2  # [N]

            # 扩展维度用于广播
            y = y  # shape: [N, M]
            y_top = y_top[:, None]  # shape: [N, 1]
            y_bottom = y_bottom[:, None]  # shape: [N, 1]

            y = np.clip(y, y_top, y_bottom)  # 防止框过高

        if index == 0:
            dx = x - s  # 相对映射坐标
            dy = center[0] - y  # y轴朝下，需倒转

            r = dy
            theta = (dx / s) * theta_0 - theta_0/2 #范围 [-theta_0 / 2, theta_0 / 2]

            # 极坐标 → 全图坐标
            x_new = center[1] + r * np.sin(theta)
            y_new = center[0] - r * np.cos(theta)
        elif index == 1:  # 右
            dx = x - s
            dy = y - s

            r = dx
            theta = (dy / s) * theta_0 - theta_0/2  # 范围 [-theta_0 / 2, theta_0 / 2]

            # 极坐标 → 全图坐标
            x_new = center[1] + r * np.cos(theta)
            y_new = center[0] + r * np.sin(theta)
        elif index == 2:  # 左
            dx = 2*s - x
            dy = y - s
        ...
        # 写回变换后坐标
        segs[..., 0] = x_new - padw  # 平移回原 patch 区域坐标
        segs[..., 1] = y_new - padh  # 平移回原 patch 区域坐标

        segs[..., 0]/= nw  # 将坐标归一化到 [0, 1]
        segs[..., 1]/= nh

        return segs

    def apply_radial_barrel_distortion(self, H, W, k=1e-6, center=None):
        """
        在全图范围内应用桶形径向畸变：r_u = r_d * (1 + k * r_d^2)
        """
        # H, W = img.shape[:2]
        if center is None:
            cx, cy = W // 2, H // 2
        ...
        r_d/= (max(H, W) / 2)  # 归一化到 [0, 1] 范围

        # 映射公式
        r_u = r_d * (1 + k * r_d ** 2 + 1e-8)

        # 归一化偏移方向
        scale = np.divide(r_u, r_d + 1e-8)  # 避免除0
        map_x = cx + dx * scale
        map_y = cy + dy * scale

        # OpenCV 的 map 必须是 float32
        map_x = map_x.astype(np.float32)
        map_y = map_y.astype(np.float32)

        return map_x, map_y

    def build_inverse_map(self, H, W, k):
        """
        构建一个反向查找表：原图位置 → 变换图位置
        返回一个 HxWx2 的映射表，map_back[y, x] = [x', y']
        """

        def build_inverse_radial_lookup_nearest(k=1e-6, max_r=1500, step=0.01):
            """
            构建从 r_u 映射到 r_d 的查找表，使得 lookup[int(r_u * 10)] 对应的 r_d 满足：
            r_d * (1 + k * r_d^2) 最接近该 r_u。
            ...
            返回:
                lookup: 一维数组，下标对应 r_u*10，值为对应最接近的 r_d
            """
            step/=(max(H, W) / 2)
            r_d_norm_max = max_r / (max(H, W) / 2)  # 归一化最大 r_d 半径
            r_d_values = np.arange(0, r_d_norm_max, step)

            # # 构造 r_d 值并计算对应 r_u
            # r_d_values = np.arange(0, max_r, step)/(max(H, W) / 2)  # 归一化到 [0, 1] 范围
            r_u_values = r_d_values * (1 + k * r_d_values ** 2)
            r_u_values*= (max(H, W) / 2)  # 恢复到原始尺度
            r_d_values*= (max(H, W) / 2)

            r_u_indices = np.round(r_u_values * 10).astype(int)
            max_index = r_u_indices.max() + 1

            # 为每个索引维护最小误差及对应 r_d
            lookup = np.full((max_index,), -1.0, dtype=np.float32)
            error_table = np.full((max_index,), np.inf, dtype=np.float32)

            for r_d, r_u, idx in zip(r_d_values, r_u_values, r_u_indices):
                approx_r_u = idx / 10.0
                error = abs(r_u - approx_r_u)
                if error < error_table[idx]:
                    error_table[idx] = error
                    lookup[idx] = r_d

            for idx in range(len(lookup)):
                if lookup[idx] == -1.0:
                    lookup[idx] = lookup[idx - 1]  # 填充未定义的值

            return lookup

        # 构建反向查找表
        lookup = build_inverse_radial_lookup_nearest(k=k, max_r=max(H, W), step=0.01)

        inverse_map = -np.ones((H, W, 2), dtype=np.float32)

        cx,cy = H//2, W//2
        for y_ori in range(H):
            for x_ori in range(W):
                dx = x_ori - cx
                dy = y_ori - cy
                ...
                scale = np.divide(r_d, r_u + 1e-8)  # 避免除0
                x_new = cx + dx * scale
                y_new = cy + dy * scale
                inverse_map[y_ori, x_ori] = [x_new, y_new]

        return inverse_map



    def _mosaic4_fish(self, labels):
        """Create a 2x2 image mosaic."""
        mosaic_labels = []
        s = self.imgsz

        key = (self.imgsz, self.imgsz)
        if key not in self.coord_cache:
            H, W = self.imgsz * 3, self.imgsz * 3
            final_mask = np.zeros((H, W), dtype=bool)
            final_src_x = np.zeros((H, W), dtype=np.float32)
            final_src_y = np.zeros((H, W), dtype=np.float32)


        yc, xc = int(s*1.5), int(s*1.5)  # mosaic center x, y
        for i in range(4):
            labels_patch = labels if i == 0 else labels["mix_labels"][i - 1]
            # Load image
            img = labels_patch["img"]
            ...
            # labels_patch_ori = deepcopy(labels_patch)  # save original labels for later use

            # Place img in img4
            if i == 0:  # top
                img4 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                x1a, y1a, x2a, y2a = max(xc + w//2+w%2 - w, 0), max(yc -s//2 - h, 0), xc + w//2+w%2, yc-s//2  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
            elif i == 1:  # right

                labels_patch['ori_shape'] = labels_patch['ori_shape'][::-1]
                label_segment = labels_patch['instances'].segments
                x = label_segment[:,: , 0].copy()
                y = label_segment[:,:, 1].copy()
                label_segment[:,:, 0] = 1 - y
                label_segment[:,:, 1] = x
                labels_patch['instances'].segments = label_segment

                img = np.transpose(img,(1,0,2))[:,::-1,:]
                h,w = img.shape[:2]

                x1a, y1a, x2a, y2a = xc+s//2, yc-h//2-h%2, min(xc + s//2 + w, s * 3), yc +  h//2
                x1b, y1b, x2b, y2b = 0, h - (y2a - y1a), min(w, x2a - x1a), h
            elif i == 2:  # left
                ...
                x1a, y1a, x2a, y2a = max(xc - s//2 - w, 0), yc - h//2 - h%2, xc - s//2, min(s * 3, yc - h//2 - h%2 + h)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), 0, w, min(y2a - y1a, h)
            elif i == 3:  # bottom
                ...
                x1a, y1a, x2a, y2a = xc - w//2 - w%2, yc + s//2, min(xc - w//2 - w%2 + w, s * 3), min(s * 3, yc + s//2 + h)
                x1b, y1b, x2b, y2b = 0, 0, min(w, x2a - x1a), min(y2a - y1a, h)

            labels_patch['img'] = img
            ...
            center_all = [(2*s,3*s//2), (3*s//2, s), (3*s//2,2*s), (s,3*s//2)]  #(y,x)轴
            theta_0 = np.pi / 180*50  # 鱼眼张角

            #鱼眼径向畸变
            if key not in self.coord_cache: #如果imgsz没变，采样可以保存下来，然后一次采样？我觉得可以，但是label就得每次都处理了
                mask, src_x, src_y = self.warp_image_to_fisheye_sector(img4, patch_box=(x1a, y1a, x2a, y2a), theta_0=theta_0, center=center_all[i],index=i)
                final_mask |= mask
                final_src_x[mask] = src_x[mask]
                final_src_y[mask] = src_y[mask]

            labels_patch['instances'].segments = self.warp_segments_to_fisheye(
                labels_patch['instances'].segments,
                patch_box=(x1a, y1a, x2a, y2a),
                theta_0=theta_0,
                center=center_all[i],
                imgsz=s * 3,  # 全图尺寸
        ...
        if key not in self.coord_cache:
            warped = img4.copy()
            for c in range(3):
                remapped = cv2.remap(img4[..., c], final_src_x, final_src_y, interpolation=cv2.INTER_LINEAR, borderValue=114)
                warped[..., c][final_mask] = remapped[final_mask]
            img4 = warped
            self.coord_cache[key] = (final_mask, final_src_x, final_src_y)
        else:
            final_mask, final_src_x, final_src_y = self.coord_cache[key]
        ...
        # k = 1e-6  # 鱼眼径向畸变系数
        if config.radial_distortion:
            k = config.radial_distortion_k
            # k = 2e-6 # 鱼眼径向畸变系数 1e-6为最佳
            # k = 0.3  # 鱼眼径向畸变系数

            # k = k/352*self.imgsz  # 根据当前图像大小调整畸变系数

            key = (self.imgsz, round(k, 8))  # 保证 float 精度误差不影响缓存命中
            if key not in self.rdial_distortion_cache:
                map_x, map_y = self.apply_radial_barrel_distortion(img4.shape[0], img4.shape[1], k=k)
                inverse_map = self.build_inverse_map(img4.shape[0], img4.shape[1], k=k)
                self.rdial_distortion_cache[key] = (map_x, map_y, inverse_map)  # 缓存映射表

                # 保证 cache 不超过最大容量
                if len(self.rdial_distortion_cache) > 10:
                    self.rdial_distortion_cache.popitem(last=False)  # 移除最早添加的项（最久未使用）
            else:
                map_x, map_y, inverse_map = self.rdial_distortion_cache[key]

            # 重映射所有通道
            distorted = img4.copy()
            for c in range(3):
                distorted[..., c] = cv2.remap(img4[..., c], map_x, map_y, interpolation=cv2.INTER_LINEAR, borderValue=114)
            img4 = distorted

            for label_i in range(len(mosaic_labels)):
                # 提取 x, y 并四舍五入取整为索引
                x = mosaic_labels[label_i]['instances'].segments[:, :, 0].copy()
                y = mosaic_labels[label_i]['instances'].segments[:, :, 1].copy()
                x = np.round(x).astype(int).clip(0, img4.shape[1] - 1)  # clip to valid range
                y = np.round(y).astype(int).clip(0, img4.shape[0] - 1)  # clip to valid range

                mosaic_labels[label_i]['instances'].segments = inverse_map[y, x]
...
class RandomPerspective:
    ...
    def affine_transform(self, img, border, ori_sz = (608,608)):
        """
        ...
        # s = random.uniform(1 - self.scale, 1 + self.scale)  ######################modified###################

        if config.cut_scale_up:
            s = random.uniform(1 - self.scale, 1.0)
        else:
            s = random.uniform(1 - self.scale, 1 + self.scale)
        # s = random.uniform(max(0.75,1 - self.scale), 1 + self.scale)

        if config.keep_label == True:
            resize_ratio = img.shape[0]/max(ori_sz[0],ori_sz[1])
            resized_h,resized_w = int(ori_sz[0]*resize_ratio), int(ori_sz[1]*resize_ratio)
            if config.keep_label_loaf == True:
                s = resize_ratio
            else:
                s = self.calculate_optimal_scale(resized_h,resized_w, a)
        # else:
        #     if config.coco and config.rapid_train_data == 'COCO':
        #         s = random.uniform(1 - self.scale, 0.7)
        #     elif config.rapid_train_data == 'HCM':
        #         s = random.uniform(1 - self.scale, 1.0)
    ...
    def calculate_optimal_scale(self, h, w , angle_degrees):
        """计算图像旋转后刚好不超出边界的最佳缩放比例"""
        # h, w = img.shape[:2]
        ...
        # 计算四个顶点在旋转后的坐标（以图像中心为原点）
        x1, y1 = -center_x, -center_y  # 左上角
        x2, y2 = center_x, -center_y  # 右上角
        x3, y3 = center_x, center_y  # 右下角
        x4, y4 = -center_x, center_y  # 左下角
        ...
        x4_rot = x4 * cos_theta - y4 * sin_theta
        y4_rot = x4 * sin_theta + y4 * cos_theta

        # 计算所有坐标的绝对值最大值
        all_coords = [abs(x1_rot), abs(y1_rot),
                      abs(x2_rot), abs(y2_rot),
                      abs(x3_rot), abs(y3_rot),
                      abs(x4_rot), abs(y4_rot)]
        max_xy = max(all_coords)

        # 计算最佳缩放比例
        # 取宽高的一半中的最小值作为基准
        base_size = max(center_x, center_y)  #反推mx_size
        scale = (base_size-1) / max_xy
    ...
    def __call__(self, labels):
        ...
        img, M, scale = self.affine_transform(img, border, labels['ori_shape'])
        ...
        if config.area_thr>0:
            i = self.box_candidates(
                # box1=instances.bboxes.T, box2=new_instances.bboxes.T, area_thr=0.01 if len(segments) else 0.10
                box1=instances.bboxes.T, box2=new_instances.bboxes.T, area_thr=config.area_thr
            )
        else:
            i = self.box_candidates(
                box1=instances.bboxes.T, box2=new_instances.bboxes.T, area_thr=0.01 if len(segments) else 0.10
                # box1=instances.bboxes.T, box2=new_instances.bboxes.T, area_thr=0.9
            )
...
class LetterBox4padding:
    ...
    def __call__(self, labels=None, image=None):
        ...
        ratio = r, r  # width, height ratios
        new_unpad = 608,608
        dw, dh = 928 - new_unpad[0], 928 - new_unpad[1]  # wh padding
...
class Albumentations_fe8kv9:
    ...
    def __call__(self, labels, p=1.0):
        if self.transform and random.random() < p:
            ...
            cls = labels["cls"]
            new = self.transform(image=im, bboxes=bboxes, class_labels=cls)  # transformed
            # im, bboxes_and_cls = new['image'], np.array([[c, *b] for c, b in zip(new['class_labels'], new['bboxes'])])
            if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                labels["img"] = new["image"]
                labels["cls"] = np.array(new["class_labels"])
                bboxes = np.array(new["bboxes"], dtype=np.float32)
...
class rapid_augmentations:
    ...
    def random_gaussian_filter(self, img):
        assert img.dim() == 3
        img = img.unsqueeze(1)
        kernel = random.choice(self.get_gaussian_kernels())
    ...
    def augment_PIL(self, img):
        # OpenCV 读取的图像是 BGR 格式，需转换为 RGB 再用 PIL
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = Image.fromarray(img_rgb)

        if np.random.rand() > 0.4:
            img = tvf.adjust_brightness(img, self.uniform(0.3,1.5))
        if np.random.rand() > 0.7:
            factor = 2 ** self.uniform(-1, 1)
            img = tvf.adjust_contrast(img, factor) # 0.5 ~ 2
        if np.random.rand() > 0.7:
            img = tvf.adjust_hue(img, self.uniform(-0.1,0.1))
        if np.random.rand() > 0.6:
            factor = self.uniform(0,2)
            if factor > 1:
                factor = 1 + self.uniform(0, 2)
            img = tvf.adjust_saturation(img, factor) # 0 ~ 3
        if np.random.rand() > 0.5:
            img = tvf.adjust_gamma(img, self.uniform(0.5, 3))

        # img_rgb = np.array(img)  # PIL.Image → numpy array（RGB, HWC, uint8）
        # img_bgr_aug = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR)  # RGB → BGR
        ...
        if not config.coco and np.random.rand() > 0.8:
        ...
        # 转回 numpy 格式：CHW -> HWC，float → uint8
        img_np = (img.numpy() * 255).clip(0, 255).astype(np.uint8)
        img_np = np.transpose(img_np, (1, 2, 0))  # (C, H, W) → (H, W, C)

        # 转回 BGR，兼容 OpenCV
        img_bgr_aug = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)

        return img_bgr_aug

    def __call__(self, labels):
        labels['img'] = self.augment_PIL(labels['img'])
        return labels

class rapid_augmentations_heavy:
    def __init__(self):
        pass
    ...
    def add_gaussian(self, imgs, max_var=0.2):
        '''
        imgs: tensor, (batch),C,H,W
        max_var: variance is uniformly ditributed between 0~max_var
        '''
    ...
    def add_saltpepper(self, imgs, max_p=0.12):
        '''
        imgs: tensor, (batch),C,H,W
        p: probibility to add salt and pepper
        '''
...
class rapid_augmentations_not_transforms:
    def __init__(self):
        pass
    ...
    def add_gaussian(self, img, max_var=0.1):
        # img: uint8 ndarray, HWC
        var = self.uniform(0, max_var)
        noise = np.random.normal(0, np.sqrt(var) * 255, img.shape).astype(np.float16)
        img = np.clip(img.astype(np.float16) + noise, 0, 255).astype(np.uint8)
        return img

    def add_saltpepper(self, img, max_p=0.06):
        # img: uint8 ndarray, HWC
        p = self.uniform(0, max_p)
        total = int(p * img.size)
        coords = [np.random.randint(0, s, total) for s in img.shape[:2]]
        salt_pepper = np.random.randint(0, 2, total) * 255

        for c in range(img.shape[2]):
            img[coords[0], coords[1], c] = salt_pepper
    ...
    def augment_PIL(self, img):
        ...
        # 3. Hue (convert to HSV and shift hue)
        if np.random.rand() > 0.7:
            hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV).astype(np.float32)
            hue_shift = self.uniform(-0.1, 0.1) * 180  # OpenCV hue range: [0,180]
        ...
        # if not config.coco and random.random() > 0.8:
        if random.random() > 0.8:
            blur_func = random.choice([
                self.random_avg_filter,
                self.max_filter,
                self.random_gaussian_filter
            ])
            img = blur_func(img)
...
class loaf_augmentations:
    def __init__(self):
        self.mean = [0.485, 0.456, 0.406]
        self.std = [0.229, 0.224, 0.225]
        self.brightness_factor = 2
        self.contrast_factor = 2
        # self.transformslist = [self.lighting_noise, self.AdjustBrightness, self.AdjustContrast]
        self.transformslist = [self.AdjustBrightness, self.AdjustContrast]

    def normalize(self, img):
        img = F.normalize(img, mean=self.mean, std=self.std)
        return img

    # def lighting_noise(self, image):
    #     '''
    ...
    #     new_image = F.to_tensor(new_image)  #不要使用 F.to_tensor() 或 .float()、.normalize() 之类会将图像转为 float 的操作；
    ...
    def lighting_noise(self, image):
        """
        交换图像的颜色通道。
        image: PIL.Image，mode="RGB"，类型为 uint8
        """
        perms = ((0, 1, 2), (0, 2, 1), (1, 0, 2),
                 (1, 2, 0), (2, 0, 1), (2, 1, 0))
        swap = perms[random.randint(0, len(perms) - 1)]
        img_np = np.array(image)  # HWC, RGB
        img_np = img_np[:, :, list(swap)]  # 通道重排
    ...
    def __call__(self, labels):
        ...
        blur_func = random.choice(self.transformslist)
        img = blur_func(img)

        # img = self.normalize(img)
        # 转回 numpy 格式：CHW -> HWC，float → uint8
        img_np = np.array(img)  # HWC, RGB, dtype=uint8
    ...
    def __call__(self, labels):
        ...
        if self.return_obb:
            labels["bboxes"] = (
                ...
                # xyxyxyxy2xywhr_limit(torch.from_numpy(instances.segments)) if len(instances.segments) else torch.zeros((0, 5))
            )
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose(
        [
            ...
            # rapid_augmentations(),
            # Albumentations_last(p=1.0),
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            # RandomHSV_last(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            # rapid_augmentations_not_transforms(),
...
def loaf_v8_transforms_ori(dataset, imgsz, hyp, stretch=False):
    """
    ...
    pre_transform = Compose([mosaic, affine])    ###############modified##################
    ...
    return Compose(
        [
            ...
            MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup),
            Albumentations_last(p=1.0),
            # Albumentations_fe8kv9(size = imgsz ),  # Albumentations_fe8kv9
            RandomHSV_last(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            RandomFlip(direction="vertical", p=hyp.flipud),
            RandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
        ]
...
def fekv9_transforms(dataset, imgsz, hyp, stretch=False):
    """
    Applies a series of image transformations for training.
...
def v8_heavy_transforms(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""
    pre_transform = Compose(
        [
            Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic),
...
def rapid_transforms(dataset, imgsz, hyp, stretch=False):
    '''custom rapid augmentations for YOLOv8 training'''
    if config.coco_pretrain_augment:
        return Compose(   ###COCO用 增强
            [   Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic),
                RandomPerspective(
                    degrees=hyp.degrees,
                    translate=hyp.translate,
        ...
        pre_transform = Compose(  ###微调用 强烈增强
            [
                RandomPerspective(
                    degrees=hyp.degrees,
                    translate=hyp.translate,
        ...
        return Compose(
            [
                pre_transform,
                # MixUp(dataset, pre_transform=pre_transform, p=0.3),
                rapid_augmentations(),
                # rapid_augmentations_heavy(),
                # Albumentations_last(p=1.0),
                # RandomHSV_last(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
...
def loaf_transforms(dataset, imgsz, hyp, stretch=False):
    '''custom rapid augmentations for YOLOv8 training'''
    return Compose(
        [
            RandomPerspective(
