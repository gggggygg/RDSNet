            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > conf_thres:
                        # label = f"{c}" if labels else f"{c} {conf[j]:.1f}"   ########################modified##########################
                        label = f"" if labels else f"{conf[j]:.1f}"   ####now_modified##########################
                        # label = ""
                        annotator.box_label(box, label, color=color, rotated=is_obb)

            elif len(classes): ####now_modified##########################
                for c in classes:
                    color = colors(c)
                    c = names.get(c, c) if names else c
