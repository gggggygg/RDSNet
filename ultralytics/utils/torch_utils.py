...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            # im = torch.empty((1, p.shape[1], stride, stride), device=p.device)  # input image in BCHW format  ###########modified############
            # flops = thop.profile(deepcopy(model), inputs=[im], verbose=False)[0] / 1e9 * 2  # stride GFLOPs

            im = torch.empty((1, p.shape[1], stride, stride), device='cuda')  # input image in BCHW format
            flops = thop.profile(deepcopy(model).to('cuda'), inputs=[im], verbose=False)[0] / 1e9 * 2  # stride GFLOPs
